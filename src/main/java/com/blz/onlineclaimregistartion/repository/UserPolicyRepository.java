...
//	List<Object> findAllByuserId(Long userId);
	...
	@Query(value = "SELECT *\r\n"
			+ "FROM policy_details as pd\r\n"
			+ "INNER JOIN user_policy as up\r\n"
			+ "WHERE up.fk_userid=:userId and pd.policy_id = up.fk_policy_id and up.fk_userid is not null;", nativeQuery = true)
...
//	List<String> findAllUserPoliciesByRoleCodeAgent(Long userId);
	...
	@Query(value = "SELECT *\r\n"
					...
					+ "	ON up.fk_policy_id = policy_id and up.user_policy_id;", nativeQuery = true)
	List<UserPolicy> findAllUserPoliciesByAdmin();
